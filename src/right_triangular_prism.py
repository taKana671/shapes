...
class RightTriangularPrism(ProceduralGeometry):
    ...
    # def __init__(self, radius=1., inner_radius=0., height=1., segs_c=3, segs_a=2, segs_top_cap=3,
    #              segs_bottom_cap=3, ring_slice_deg=0, slice_caps_radial=1, slice_caps_axial=1, invert=False):
    def __init__(self, adjacent=3, opposite=4, inner_adjacent=1.5, inner_opposite=2,
                 height=1., segs_a=2, segs_top_cap=3, segs_bottom_cap=3, invert=False):
        super().__init__()
        self.adjacent = adjacent  # bottom edge of right triangle
        self.opposite = opposite  # height of right triangle
    ...
    def calc_radius(self, adjacent, opposite):
        return (adjacent ** 2 + opposite ** 2) ** 0.5 * 0.5


    def get_geom_node(self):
        # self.radius = (self.adjacent ** 2 + self.opposite ** 2) ** 0.5 * 0.5
        ...
        self.segs_c = 3
        self.slice_rad = math.pi * 180 / 180


        # self.ring_slice_deg = 90
        # self.delta_rad = math.pi * ((360 - self.ring_slice_deg) / 180) / self.segs_c
        # self.delta_rad = math.pi * (90 / 180) / self.segs_c
        ...
        # self.delta_radius = (self.opposite - self.adjacent) / self.segs_c
        ...
        # when not self.invert, replace self.inner_oposite and self.innter_adjacent
        if self.inner_radius:
            cylinder_maker = RightTriangularPrism(
                self.inner_opposite, self.inner_adjacent, 0.0, 0.0,
                self.height, self.segs_a, 0, 0, not self.invert)
